...
const CST = Carmentis.CST;
const dom = Carmentis.util.dom;
const textEncoder = Carmentis.util.textEncoder;
...
function initializeUi(options) {
    ...
    // bottom-right icon
    /*body.append(
        dom.create("div", "_app_icon").className("_app_icon_"  + position).append(
            dom.create("div", "_app_open").append(
                icon("phone")
            )
        )
...
async function requestBlockData(request) {
    ...
    screenSet(
        ...
        dom.create("button").className("_app_button_small").text("See history").click(_ => _),
        dom.create("button").className("_app_button_small").text("See details").click(_ => _),
        dom.create("hr"),
...
async function confirmDevice(request) {
    ...
    let expectedDomain = "filesign.acme.me";
...
function accountList() {
    screenSet(
        ...
        ...application.accounts.map((acct, n) =>
            dom.create("div").append(
                dom.create("button").className("_app_account_btn").append(
...
async function loadAccounts() {
    let data = await storage.read("accounts");

    application.accounts = JSON.parse(textEncoder.decode(data));
}
...
async function saveAccounts() {
    let data = textEncoder.encode(JSON.stringify(application.accounts));

    await storage.write("accounts", data);
}
...
async function loadMicroBlock(nonce) {
    ...
    let res = await Carmentis.processRecord(flow, nonce, textRendering, fieldRendering);
