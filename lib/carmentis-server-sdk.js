/**
 ...
 Built 2024-08-07T08:32:12.389Z
 --
...
const DATA_URL = "https://carmentis.local";
const NODE_URL = "https://node.carmentis.local";
const ROOT_ACCOUNT_PUB_KEY$1 = "0362E86E0F25B68D483F4A9A2D461D74DD3050A03AE403252DD8A24E543C419095";
...
const SECTION_ORGANIZATION = {
    ...
    [ S_ORG_DESCRIPTION ] : {
        public: [
            ...
            { name: "country", type: STRING$1, size: 2 },
...
async function serialize(schema, obj) {
    return await encode(schema, obj);
}
...
//  encode()                                                                                                                    //
// ============================================================================================================================ //
async function encode(schema, obj, header = null) {
...
async function nodeQuery(msgId, msgData) {
    console.log("nodeQuery", msgId, msgData);
...
function fileObject(param) {
    async function uidToHash() {
        return await sha256(encode$1(param.prefix + param.uid));
    }
...
//  create()                                                                                                                    //
// ============================================================================================================================ //
function create$4(organizationId, qrId, publicKey) {
...
async function request(arg) {
    ...
    let endpoint = "https://operator.acme.me";

    return await new Promise(function(resolve, reject) {
        let socket = getSocket(endpoint, connect, async (...arg) => dataCallback$1(socket, ...arg));
...
async function sendRequest(socket) {
    let iv = getRandomBytes(16),
        requestStr = encode$1(JSON.stringify(socket.requestObject.data)),
...
function qrStatusConnected(elementId) {
    qrSetContent(
        ...
        [
            ...
            create$5("p").style({ fontSize: "48px", margin: 0 }).html(`<i class="bi bi-check-circle-fill"></i>`),
            create$5("p").text("Connected"),
            create$5("p").style({ fontSize: "13px" }).text("Please follow the instructions on your wallet.")
        ]
    );
}
...
function qrStatusExpired(elementId) {
    qrSetContent(
        ...
        [
            // TODO: use a self-contained icon
            create$5("p").style({ fontSize: "48px", margin: 0 }).html(`<i class="bi bi-arrow-clockwise"></i>`),
            create$5("p").text("Expired"),
            create$5("p").text("(Click to refresh)")
        ],
        refresh
    );
...
function setQRCode(elementId, organizationId, qrId, publicKey) {
    let [ qrImgTag, qrData ] = create$4(organizationId, qrId, publicKey);
...
async function createGenesis(objectType) {
    return await create$3(true, objectType);
}
...
async function createNext(objectType) {
    return await create$3(false, objectType);
}
...
// ============================================================================================================================ //
async function create$3(isGenesis, objectType) {
    let block = {
        isGenesis: isGenesis,
        sections : [],
        dataSize : 0,
...
async function prepareBlockArray(block) {
    ...
    if(block.isGenesis) {
        write48(block.array, 1, OFFSET_NONCE$1);
        block.nonce = 1;
        write8(block.array, block.type, OFFSET_TYPE);
        block.array.set(block.seed, OFFSET_SEED);
    }

    write48(block.array, block.ts, OFFSET_TIMESTAMP$1);
...
// ============================================================================================================================ //
async function finalizeHeader(block, sigPrivKey, prevNonce, prevHash) {
    // -------------------------------------------------------------------------------------------------------------------------- //
    //  if this is not a genesis block, set the current nonce and previous block hash                                             //
    // -------------------------------------------------------------------------------------------------------------------------- //
    if(!block.isGenesis) {
        write48(block.array, prevNonce + 1, OFFSET_NONCE$1);
        block.array.set(prevHash, OFFSET_PREV_HASH$1);
...
//  load()                                                                                                                      //
// ============================================================================================================================ //
async function load$2(id, microBlockList) {
...
function parse(appDef, msg) {
    return msg.split(/(\{\{.+?\}\})/g).map((part, ndx) =>
        ndx & 1 ?
...
//  create()                                                                                                                    //
// ============================================================================================================================ //
function create$1(type, objectCallback, keyring, sectionCallback) {
    let chain = create$2(type);
...
//  load()                                                                                                                      //
// ============================================================================================================================ //
async function load$1(type, chainId, objectCallback, keyring, sectionCallback) {
...
async function loadBlock(obj, callback, blockData) {
    ...
    await decodeHeaderContent$1(block, blockData);
...
function create(keys) {
    return create$1(OBJ_FLOW, populateProperties, keys, processSection);
}
...
async function load(id, keys) {
    return await load$1(OBJ_FLOW, id, populateProperties, keys, processSection);
}
...
//  getFlatRecord()                                                                                                             //
// ============================================================================================================================ //
function getFlatRecord(appDef, fieldDef, record) {
    let arr = [];
...
async function processRecord$1(obj, nonce, textCallback, fieldCallback) {
    ...
    msg.split(regex).forEach((str, i) => {
        if(i & 1 && str) {
            fields.push(str);
        }
    });

    for(let ndx in fields) {
        let name       = fields[ndx],
            recordName = name.replace(/.+?\./, ""),
            msgObj     = msgFields[ndx],
            data       = getMicroBlockRecord(obj, msgObj.nonce),
            record     = await decode$2(appDef, data.record),
            flatRecord = getFlatRecord(appDef, appDef.field, record),
    ...
    msg.split(regex).forEach((str, i) => {
        if(str) {
            if(i & 1) {
                fieldCallback(str, items[str].def, items[str].value);
            }
            else {
                textCallback(str);
            }
...
const debug = {
    dataServerQuery: async (...arg) => await dataServerQuery(...arg)
};

export { constants$1 as CST, constants, debug, deriveAccountPrivateKey, deriveActorPrivateKey, deriveAesKeyFromPassword, deriveAuthenticationPrivateKey, deriveNodePrivateKey, deriveOrganizationPrivateKey, derivePepperFromSeed, deriveUserPrivateKey, generateWordList, getChainStatus, getMasterBlock, getMasterBlockList, getMatchingWords, getMicroBlock, getMicroChain, getPublicKey, getSeedFromWordList, getWordListFromSeed, prepareApproval, processRecord, util, wallet };
