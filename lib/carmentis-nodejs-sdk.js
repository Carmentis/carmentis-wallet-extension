/**
  ...
  Built 2024-10-24T14:43:32.544Z
  --
...
// free sandbox credit, in exchange
...
// ---------------------------------------------------------------------------------------------------------------------------- //
//  Notes:                                                                                                                      //
//    - Permissions are stored as a 32-bit integer.                                                                             //
//    - The administrator flag MUST be the least significant bit.                                                               //
// ============================================================================================================================ //
const PERM_COUNT = 9;
const PERM_ALL = (1 << PERM_COUNT) - 1;

const PERM_ADMIN       = 0x0001;
const PERM_EDIT_ORG    = 0x0002;
const PERM_PUBLISH_ORG = 0x0004;
const PERM_ADD_USER    = 0x0008;
const PERM_REMOVE_USER = 0x0010;
const PERM_CREATE_APP  = 0x0020;
const PERM_EDIT_APP    = 0x0040;
const PERM_DELETE_APP  = 0x0080;
const PERM_PUBLISH_APP = 0x0100;
...
var user = /*#__PURE__*/Object.freeze({
  ...
  PERM_ADD_USER: PERM_ADD_USER,
  PERM_ADMIN: PERM_ADMIN,
  PERM_ALL: PERM_ALL,
  PERM_COUNT: PERM_COUNT,
  PERM_CREATE_APP: PERM_CREATE_APP,
  PERM_DELETE_APP: PERM_DELETE_APP,
  PERM_EDIT_APP: PERM_EDIT_APP,
  PERM_EDIT_ORG: PERM_EDIT_ORG,
  PERM_PUBLISH_APP: PERM_PUBLISH_APP,
  PERM_PUBLISH_ORG: PERM_PUBLISH_ORG,
...
//  get()                                                                                                                       //
// ============================================================================================================================ //
function get(q, throwError = true) {
...
//  getSection()                                                                                                                //
// ============================================================================================================================ //
async function getSection$1(block, id, key) {
...
async function getLastSection(type, microchainId, sectionId) {
  ...
  let [ data ] = await getSection$1(microBlock, sectionId);
  ...
  for(let ndx in arr) {
    ...
    for(let blockId = minBlock; blockId <= maxBlock && !found; blockId++) {
      let microBlock = microBlocks.find(o => o.nonce == currentNonce - blockId),
          [ publicSection ] = await getSection$1(microBlock, S_FLOW_PUBLIC_DATA);
...
// ============================================================================================================================ //
/**
 * Derives and returns a seed from a provided list of words.
 *
 * @param words The words from which the seed is derived.
 *
 * @throws string if a provided word does not belong to a finite set of words.
 *
...
async function loadFromChain(flowId, actorId, actorKey) {
  ...
  let [ declaration ] = await getSection$1(genesisBlock, S_FLOW_DECLARATION);
 ...
 *      }>}
 */
function loadPublicDataFromMicroBlock$1(applicationId, flowId, nonce) {
  return prepareApproval$1(applicationId, flowId, [])
      .then(flow => {
        return processRecord$1(flow, nonce)
      })
}
...
const constants = {
  ...
  // CST.DATA
  SECTION_NAME: SECTION_NAME,
  FLOW_PUBLIC_DATA: S_FLOW_PUBLIC_DATA,
 ...
 *
 * @type {(function(id : Uint8Array): Promise<{
 *     status: number,
 *     currentBlock: null,
 *     currentNonce: number,
 *     type: number,
 ...
 *
 * @type {(function(id : Uint8Array) :  masterBlock: number,
 *       version: number,
 *       nonce: number,
 *       hash: Uint8Array,
 *       prevHash: Uint8Array,
 ...
 *      }>}
 */
const loadPublicDataFromMicroBlock = userFunction(loadPublicDataFromMicroBlock$1);
 ...
 *
 * @type {(function(nbWords : number): string[])}
 *
 * @returns string[] A nbWords-sized list containing the generated words.
 */
const generateWordList               = userFunction(generateWordList$1);
const getSeedFromWordList            = getSeedFromWordList$1;
 ...
 * Derives and returns the private key from a seed and a nonce.
 *
 * @param seed {Uint8Array} The seed used to derive the private key.
 * @param nonce {number} The nonce to derive a different private key from the same seed.
 *
 ...
 *  )}
 */
const getSection         = userFunction(getSection$1);
