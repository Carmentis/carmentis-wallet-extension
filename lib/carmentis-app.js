...
function userData(n) {
    ...
    if(verifyEmailButton) {
        screenAppend(
            dom.create("button").className("_app_mt16").text("Verify your email").click(async function () {
                console.log("Open verify email page");
                const port = browser.runtime.connect({name: 'carmentis-wallet'});
                console.log("kiki", storage.getBits())
                port.postMessage({ function: "verifyEmail", data: {
                        n,
                        encryptedKey: uint8.toHexa(storage.getBits())
                    }
                });
            })
        );
...
//  verifyEmail()                                                                                                               //
// ============================================================================================================================ //
export async function verifyEmail(n=0, refresh = true) {
    console.log("verifyEmail", n, refresh);
    let acct = getAccount(n);

    /*if(browser.extension.getViews({ type: "popup" }).length > 0) {
        console.log("No tab views");
        return;
    } else {
        console.log("Executing in tab ?");
    }*/
    ...
    console.log("otpToken", application.otpToken);
