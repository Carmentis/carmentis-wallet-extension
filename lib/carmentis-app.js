...
async function showMicroblock(microblock) {
    ...
    flow = mc.flowObject;
    const blocksOfChain = flow.chain.microBlock;
    const carouselData = [];
    ...
    carouselData.forEach((datum) => {
        const datumInDom = dom.create("div").append(
            dom.create("h5").text("Microchain: " + datum.microchain.substring(0, 8) + '...'),
    ...
    carouselEl.addEventListener('slide.bs.carousel', function (evt) {
        //console.log('slide.bs.carousel', evt);
    });
