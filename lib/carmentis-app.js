...
async function initDB() {
    db = await window.indexedDB.open("carmentis", DB_VERSION);
...
function initWS() {
    if(!application.config?.nodeEndpoint) {
        console.log("[ws] No node endpoint defined. Doing nothing.");
        return;
    }
    if(ws) {
        ws.close();
    ...
    ws.onmessage = (event) => {
        ...
        if (data.result && data.result.events && data.result.events['microblock.hash']) {
            const microblockEvent = {
                hash: data.result.events['microblock.hash'][0],
...
async function approval(request) {
    ...
    async function confirm() {
        ...
        if(res.microBlockId){
            const request = objectStore.get(res.microBlockId);
            ...
            request.onsuccess = function(event) {
                ...
                if(microblock) {
                    ...
                    objectStore.add({
                        hash: res.microBlockId,
                        microchain: res.flowId,
                        nonce: res.nonce,
                        status: "pending",
                        ts: (new Date()).getTime()
                    });
