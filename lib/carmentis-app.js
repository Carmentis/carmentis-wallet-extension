...
function getMicroblocksOrderedByTimestamp(callback) {
    ...
    request.onsuccess = function(event) {
        callback(request.result); // Les transactions sont triées par timestamp
    };
        ...
        if (data.result && data.result.events && data.result.events['microblock.hash']) {
            ...
            request.onsuccess = function(event) {
                ...
                if(microblock) {
                    ...
                    microblock.status = "validated";
                    microblock.data = data.result.data.value.TxResult.tx;
        ...
        microblocks.forEach((microblock) => {
            ...
            dom.get("#_app_history_list").append(
                dom.create("li", "_app_history_item_"+microblock.hash)
                    ...
                    ).click(_ => showMicroblock(microblock))
...
async function showMicroblock(microblock) {
    const blocksOfChain = (await loadFromMicrochain(microblock.microchain))?.flowObject.chain.microBlock;
    const carouselData = [];

    blocksOfChain.forEach((block) => {
        carouselData.push(
            {
                microchain: microblock.microchain,
                nonce: microblock.nonce,
                status: microblock.status,
                timestamp: (new Date(microblock.ts)).toLocaleString()
            }
        )
    });

    const carouselBody = dom.get(".carousel-inner");
    carouselBody.innerHTML = ''; // Vide le contenu existant

    console.log('carouselData', carouselData)

    carouselData.forEach((datum) => {
        const datumInDom = dom.create("div").append(
            dom.create("h5").text("Microchain: " + datum.microchain),
            dom.create("p").text("Nonce: " + datum.nonce),
            dom.create("p").text("Status: " + datum.status),
            dom.create("p").text("Timestamp: " + datum.timestamp)
        );
        carouselBody.append(
            dom.create("div")
...
async function getAccountPublicKey(n) {
    console.log('getAccountPublicKey', application, n);
    let pepper  = await Carmentis.derivePepperFromSeed(application.seed, n + 1),
