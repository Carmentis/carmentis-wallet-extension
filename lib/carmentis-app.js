...
async function showMicroblock(microblock) {
    console.log("showMicroblock");
    const blocksOfChain = loadFromMicrochain(toHexa(new Uint8Array(microblock.microchain)))?.flowObject?.microBlock;
    const carousel = [];
    ...
    const modalBody = dom.get("#microblockModalBody");
    modalBody.innerHTML = ''; // Vide le contenu existant

    dataToDisplay.forEach(({ label, value }) => {
        modalBody.append(
            dom.create("div").append(
                dom.create("span").text(label + ": "),
                dom.create("span").text(value)
