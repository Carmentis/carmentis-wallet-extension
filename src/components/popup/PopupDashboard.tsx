...
import ActionMessagePage from "@/src/components/popup/ActionMessagePage.tsx";
...
export function PopupDashboard() {
    ...
    function executeBackgroundTask( task : () => void ) : Promise<void> {
    ...
    function AbortWithError( error : string ) {
        setError(error)
            ...
            Carmentis.wallet.getRequestByQRCode(QRCodeData)
                .then(request => {
                    ...
                    if (request) {
                        ...
                        AbortWithError("Returned response is empty.")
                    }
                })
        });
            ...
            { !showWaitingScreen && !currentActionMessage.isEmpty() &&
                <>
                    ...
                    { actionRequestState == RequestTreatmentState.EVENT_APPROVAL &&
                        <EventRequestApproval onAccept={handleRequestEventApproval} onReject={RejectRequest}></EventRequestApproval>
                    }
